...
<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      <el-select
        v-model="selectValue_s"
        placeholder="请选择品牌分类"
        style="margin: 0 0 0 30px;"
        @change="toSelectClass_s"
      >
        <el-option
          v-for="item in options_s"
          :key="item.value"
...
import {
  getPigPriceList_api,
  addPigPriceList_api,
  editPigPriceList_api,
  deletePigPriceList_api
} from "@/api/buyPigList";
export default {
  created() {
    //获取列表
  ...
  methods: {
    ...
    getSomeList(data) {
      ...
      getPigPriceList_api(getData).then(res => {
    ...
    onYesEdit() {
      ...
      editPigPriceList_api(data).then(res => {
        console.log(res);
        if (!res.status) {
          this.dialogVisible = false;
          this.getSomeList({});
    ...
    onSubmit() {
      ...
      addPigPriceList_api(data).then(res => {
        console.log("add", res);
        if (!res.status) {
          this.dialogVisible = false;
          this.getSomeList({});
