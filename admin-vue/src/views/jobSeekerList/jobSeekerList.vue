...
<template>
	<div class="app-container">
        ...
        <el-table-column label="期望地区" prop="address">
			</el-table-column>
		<el-table-column label="薪资要求" prop="salary">
			</el-table-column>
    <el-table-column label="岗位工龄" prop="exper">
			</el-table-column>
...
<script>
import {
  getJobSeekerList_api,
  getJobSeekerInfo_api,
  deleteJobSeekerList_api,
  setTopping
} from "@/api/buyPigList";
export default {
  created() {
  ...
  methods: {
    ...
    toLookInfo(index) {
      ...
      getJobSeekerInfo_api({ id: this.tableData[index].id }).then(res => {
    ...
    toDelete(index) {
      //删除
      deleteJobSeekerList_api({ id: this.tableData[index].id }).then(res => {
        console.log(res);
        this.getSomeList({});
      });
    },
    getSomeList(data) {
      ...
      getJobSeekerList_api(getData).then(res => {
